#!/usr/bin/env python3
"""Replay existing conversations through C–F on hermes_test (not live).

Copies `conversations` from hermes_memory, MERGEs the AGE flower, then runs
extract_nouns / passports / mentions using stored turn embeddings and fresh
label embeddings. Does not insert duplicate conversation rows on live.
"""
...
async def replay(test_dsn: str) -> dict[str, int]:
    cfg = load_config()
