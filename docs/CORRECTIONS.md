...
| 12 | `hermes-memory-backfill` performs the live C–F drain (nouns/mentions/passports) for historical turns. | It only writes AGE `Concept`/`ABOUT` edges via the legacy linker. The console script is now `hermes-memory-backfill-about`; the old name exits 2. It does not run `extract_nouns`, passports, `mentions`, or stamp `drain_status`. The actual C–F backfill tool is `python scripts/replay_conversation_manifold.py --live`. | Issue #73; `backfill.py`; `replay_conversation_manifold.py` |
| 13 | A dev clone's local `.env` (or absence of one) safely inherits or defaults to the correct database. | Missing `HYBRID_AGE_DSN` after dotenv load **raises**. Clone `.env` is loaded first (Documents `:5450`), then `~/.hermes/.env` (install `:5452`). There is no silent `_DEFAULT_DSN` fallback. | Issue #74; `config.py` `load_config` / `load_dotenv_files` |

| 14 | `File.beamScore: float` is a stored column already populated from `LEDGER`/`drain_status` and can be surfaced in the codebase-ingest pane. | No such column exists. `beam_score` is a **query-time** `beam_score()` over `noun`/`semantic_edge` (`src/hermes_memory/walk.py:76-91`, `c = src_align * tgt_align`, `composite = 0.4*sim +0.4*c*prov+0.2*decay`), `LEDGER` is a process-local write-outcome counter, and `drain_status` is a per-`conversations` row flag (`sql/init/02_schema.sql:21`, `V11__drain_status.sql`). None produce a storable `File.beamScore`. Fabricated in `docs/research/swarm-2026-09-08/synthesis-matrix.md:17` and `graph-vector-db-brief.md:137`. | Peer review 2026-09-08; `walk.py:76-91`, `02_schema.sql:21`, `grep -R File.beamScore` nil in migrations |
