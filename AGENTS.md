...
- `scripts/` — librarian CLI, extractors, watchdogs. `hermes-memory-backfill-about` writes leftover AGE Concept/ABOUT only (the old name `hermes-memory-backfill` exits 2). Live C–F (flower, nouns, passports, mentions, `drain_status`) is `python scripts/replay_conversation_manifold.py --live` against `hermes_memory`. Do not use the ABOUT script for manifold backfill.
- `hermes-memory-install` pins GitHub `main` when there is no checkout (tarball; never implicit `v0.1.0` or `HEAD`), or `origin/main` via `git archive` from a clone. It stamps `~/.hermes/plugins/hybrid-age/.hermes-memory-version`, and owns `hermes_memory_installed` on `:5452`. It must not symlink the plugin at a clone, `pip install -e` the working tree, default to `:5450`/`hermes_memory`, or `cp .env.example` into `$HOME`.
...
- Compose init is first-boot only. GitHub CD does not migrate live (loopback Postgres). Provider `_ainit`, pane boot, and ingest call `apply_pending_migrations` then `Store.require_schema_head()` (backstop). Topology is one provider plus an optional pane, not a rolling fleet.
- Do not lock a live-shaped retrieval golden set until the V9-gap turns are C–F backfilled. `assert_live_shaped_eval_allowed` refuses `eval_kind=live_shaped` while unpassported turns remain (or the count is unknown). That count is the passport anti-join (historical: row exists, never stamped a passport) — not `conversations.drain_status` (prospective: this drain's C–F after insert B). `DEPLOY_TOPOLOGY.head_check` is `expected_not_applied_only` and sits next to `rolling_deploy`; flipping the latter requires changing the former.
