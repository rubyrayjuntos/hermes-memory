...
Status column is a **snapshot against `origin/main` @ `8c86181` (2026-09-07)**, with live evidence from the installed pin `8c3e0de` on `:5452` / `hermes_memory_installed`.
...
| ID | Track | Bar | Criterion | Why it is a row (already surfaced) | Status @ 8c86181 |
|----|-------|-----|-----------|------------------------------------|------------------|
| `D-MVP-1` | Delivery | MVP | **Turn-drain writes** fail loud **or** are durably marked. L0 never raises. L1 = no row + process `writes_failed` (accepted loss). L2 = row + `embed_null`. L3 = row + `drain_status='graph_degraded'`. Scope is `insert_turn` / flower / nouns / mentions **except** the two named holes below. | Silent loss was the original lie. | **Met** for this narrowed drain. Installed path: 4/4 turns `drain_status='complete'`; `/api/health` `writes_failed=0` `embed_null=0` `graph_degraded=0`. CI green at `8c86181` ([34181809245](https://github.com/rubyrayjuntos/hermes-memory/actions/runs/34181809245)). **Does not cover** [#70](https://github.com/rubyrayjuntos/hermes-memory/issues/70) / `D-MVP-6` or [#71](https://github.com/rubyrayjuntos/hermes-memory/issues/71) / `D-MVP-7`. |
...
| `D-MVP-3` | Delivery | MVP | CI on every merge runs the write-path tests (L1/L3 names collectable; `integration or store or idempotent` including `test_drain_status`). | A red `main` with “36 passed” is not a green write path until the failure is opened. | **Met** on current `origin/main` `8c86181` — CI [34181809245](https://github.com/rubyrayjuntos/hermes-memory/actions/runs/34181809245) success (ruff+mypy, property 3.11/3.12, integration). Re-opened 2026-09-07; not the older `6704b07` run. |
...
| `D-MVP-5` | Delivery | MVP | **Ship-blocker ([#74](https://github.com/rubyrayjuntos/hermes-memory/issues/74)).** After an explicit dotenv load, a missing `HYBRID_AGE_DSN` (or yaml-named `dsn_env`) must **raise**. It must not fall back to `_DEFAULT_DSN` `:5450/hermes_memory` while that container still answers. | Same failure shape as I-INSTALL-1, one layer out. | **Open — blocks ship.** Still reproducible at `8c86181`. **Acceptance check not done:** Documents still has **no** `.env` (only `.env.example`). Do not merge fail-loud until that file exists and the Documents pane start path loads it, not `_DEFAULT_DSN` / not silent `~/.hermes/.env` reuse. |
| `D-MVP-6` | Delivery | MVP | **[#70](https://github.com/rubyrayjuntos/hermes-memory/issues/70).** Dim mismatch on an existing `semantic_edge` UPDATE must fail loud or stamp `graph_degraded` / LEDGER — not `continue`. | Same invariant as `D-MVP-1`, on the mentions EMA path that `D-MVP-1` no longer pretends to cover. | **Open.** Still reproducible at `8c3e0de`/`8c86181` (`store.py:623-624`). Does **not** un-Met the narrowed `D-MVP-1`. Blocks a fully honest “every write-path failure is loud” cut. |
| `D-MVP-7` | Delivery | MVP | **[#71](https://github.com/rubyrayjuntos/hermes-memory/issues/71).** Ingest stamps `embed_model`/`embed_dim` on `doc_chunks` and file-backed `memory_entries`. `trust_nomic_768` stays legacy-NULL only. | New NULL rows keep growing the “legacy” set. | **Open.** Still reproducible (`ingest.py:516-524`, `:551-569`). Ingest is not the turn drain; it is still a writer. |
...
| `RQ-MVP-2` | Retrieval Quality | MVP | Synthetic ANN / verify path passes in CI (`hermes-memory-verify`, walk/score tests). This proves the *pipe*, not that recall is good. | Needed so Delivery hardening cannot ship a broken retriever unnoticed. | **Met** as mechanism smoke on `8c86181` CI [34181809245](https://github.com/rubyrayjuntos/hermes-memory/actions/runs/34181809245). **Not** a quality floor. Installed-path recall: `/api/librarian/search?q=Nightingale` on `:5452` returned conv 5–8. |
...
CI on docs commit `8c86181`: [34181809245](https://github.com/rubyrayjuntos/hermes-memory/actions/runs/34181809245) green. Pin `8c3e0de`: [34162339003](https://github.com/rubyrayjuntos/hermes-memory/actions/runs/34162339003) green.

**MVP cut is not Met.** Open MVP rows: `D-MVP-5` (#74 + Documents `.env` still missing), `D-MVP-6` (#70), `D-MVP-7` (#71), `OC-MVP-3` (meaning bar, needs level-6 graph artifact), `OC-MVP-4` (pane interaction spec §§1–3). [#73](https://github.com/rubyrayjuntos/hermes-memory/issues/73) is naming only — does not gate any row.
