...
| Does a missing DSN silently landing on `:5450` block ship? | **Yes.** That is `D-MVP-5` / [#74](https://github.com/rubyrayjuntos/hermes-memory/issues/74). Fail-loud does not ship until Documents names `:5450` on purpose. |
...
Status column is a **snapshot against `origin/main` @ `6704b07` (2026-09-07)**, with live evidence from the installed pin `8c3e0de` on `:5452` / `hermes_memory_installed`.
...
| ID | Track | Bar | Criterion | Why it is a row (already surfaced) | Status @ 6704b07 |
|----|-------|-----|-----------|------------------------------------|------------------|
| `D-MVP-1` | Delivery | MVP | Writes fail loud **or** are durably marked. L0 never raises. L1 = no row + process `writes_failed` (accepted loss). L2 = row + `embed_null`. L3 = row + `drain_status='graph_degraded'`. | Silent loss was the original lie. | **Met** under the OR on the installed path: 4/4 turns `drain_status='complete'`, pane `/api/health` `writes_failed=0` `embed_null=0` `graph_degraded=0`. Remaining silent holes that do **not** un-Met this OR today: [#70](https://github.com/rubyrayjuntos/hermes-memory/issues/70) (dim-mismatch `continue`), [#71](https://github.com/rubyrayjuntos/hermes-memory/issues/71) (ingest NULL stamps). L1 is still not a durable mark. |
| `D-MVP-2` | Delivery | MVP | Boot refuses to start on schema drift (`apply_pending_migrations` then `Store.require_schema_head`). | Compose init is first-boot only; live migrate is a backstop. | **Met.** Installed plugin has no copied `sql/` tree. Head check is `CONSUMER_HEAD_VERSIONS` vs live `migration_history` (`8c3e0de`). `:5452` history tonight: V1–V3, V6–V11. |
| `D-MVP-3` | Delivery | MVP | CI on every merge runs the write-path tests (L1/L3 names collectable; `integration or store or idempotent` including `test_drain_status`). | A red `main` with “36 passed” is not a green write path until the failure is opened. | **Met** on pin `8c3e0de` (run 34162339003) and map commit `6704b07` (run 34166580673). `f154df5` merged-but-red remains historical. |
...
| `D-MVP-5` | Delivery | MVP | **Ship-blocker ([#74](https://github.com/rubyrayjuntos/hermes-memory/issues/74)).** After an explicit dotenv load, a missing `HYBRID_AGE_DSN` (or yaml-named `dsn_env`) must **raise**. It must not fall back to `_DEFAULT_DSN` `:5450/hermes_memory` while that container still answers. | Same failure shape as I-INSTALL-1, one layer out: a live but unintended DB looks like empty pane / missed recall. | **Open — blocks ship.** Still reproducible at `8c3e0de` (`config.py:27-90`, `graph_runtime.py:68-109`). **Do not merge the fail-loud fix until** Documents has its own explicit `:5450` DSN and pane start path (acceptance comment on #74, 2026-09-07). |
...
| `RQ-MVP-1` | Retrieval Quality | MVP | One named, documented scoring function (`beam_score` in walk / architecture / manifold spec). | Same split-brain as `D-MVP-4`. | **Met.** Construction ceiling [#72](https://github.com/rubyrayjuntos/hermes-memory/issues/72) (adjacent pairs + `MAX_NOUNS=5`) is RQ shape, not a second formula. |
| `RQ-MVP-2` | Retrieval Quality | MVP | Synthetic ANN / verify path passes in CI (`hermes-memory-verify`, walk/score tests). This proves the *pipe*, not that recall is good. | Needed so Delivery hardening cannot ship a broken retriever unnoticed. | **Met** as mechanism smoke on `8c3e0de` / `6704b07` CI. **Not** a quality floor. Installed-path recall tonight: `/api/librarian/search?q=Nightingale` on `:5452` returned conv 5–8 (cross-session Nightingale / Solenoid / Kilnbrook). |
...
CI on the pin: [34162339003](https://github.com/rubyrayjuntos/hermes-memory/actions/runs/34162339003) green. Schema-map commit `6704b07`: [34166580673](https://github.com/rubyrayjuntos/hermes-memory/actions/runs/34166580673) green.

Ship is blocked by `D-MVP-5` / [#74](https://github.com/rubyrayjuntos/hermes-memory/issues/74), not by these Met rows.
